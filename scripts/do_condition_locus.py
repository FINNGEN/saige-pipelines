...
def run_cond(null, var_ratio, bgen,bgen_idx,sample, chr, start, stop, output, condition_on, minmac, script):
    ...
    # currently saige output doubles variantid but does not give correct number of headers
    p = subprocess.Popen(f'cat <(head -n 1 {output}) <(cut -f 1-3,5- -d" " {output} | tail -n+2) >  {output}_fix', shell=True, executable='/bin/bash')
    p.wait()
    os.system(f'mv {output}_fix {output}')
...
if __name__ == '__main__':
    ...
    out = f'{args.out_prefix+"_" if len(args.out_prefix)>0 else ""}{os.path.basename(args.null_file)}_{args.condition_snp}.conditional'
    ...
    summary_out = f'{args.out_prefix+"_" if len(args.out_prefix)>0 else ""}{os.path.basename(args.null_file)}_independent.snps'
    ...
    with open( summary_out, 'w' ) as summary:
        ...
        while not r.empty:
            ...
            out = f'{os.path.basename(args.null_file)}_{"-".join(cond_snps)}.conditional'
            run_cond(args.null_file, args.variance_ratio_file, args.bgen_file, args.bgen_index, args.sample_file, args.chr,
                     args.start, args.stop, out, cond_snps, args.min_mac, args.saige_script )
